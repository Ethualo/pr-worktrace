...
AI PR 리뷰 봇 + 작업 기록 로거, GitHub Action으로 패키징. 데이터베이스 없음 — GitHub 자체(PR 댓글, 리액션, 답글 스레드)가 유일한 상태 저장소라, 매 실행은 상태를 GitHub에서 다시 읽어 재구성하는 무상태(stateless) 프로세스다.

## 왜 만들었나

대부분의 PR 리뷰 봇은 이미 무슨 말을 했는지 기억하려고 백엔드가 필요하다. worktrace-bot은 그렇지 않다. 자신이 올리는 결과물 자체에 상태를 인코딩한다 — 리뷰 댓글마다 숨긴 HTML 주석 마커를 달고, 봇 자신의 "사유를 남겨달라" 답글에도 숨긴 마커를 달아, 다음 실행 때 그 마커들을 다시 파싱해 상태를 되살린다. 운영할 인프라도, 낡아서 못 쓰게 될 인프라도 없다.
...
- **`mode: poll`** — LLM 안 씀. 게시된 댓글에서 리뷰 상태 재구성, 거절된 제안엔 사유 요청(봇 답글에 `<!-- worktrace-reason-request -->` 마커 달아 다음 실행에서 봇 답글 vs 사람 답글 구분), PR 종료 시 `.worklog/*.md` 요약 커밋. `createProvider`는 review 모드 안에서만 동적 임포트되므로 poll 모드는 API 키 자체가 필요 없음.
...
**NVIDIA NIM**은 이 `openai` 프로바이더를 그대로 통과한다 — 코드베이스 어디에도 NIM 전용 분기 없음. NIM은 기본으로 추론(chain-of-thought)이 켜져 있어 응답 앞에 붙는데, 이건 설정만으로 끈다:
...
서로 다른 LLM 키, 서로 다른 기본 브랜치(`main` vs `master`)를 쓰는 독립된 두 저장소에서 종단 간(E2E) 검증 완료.
...
테스트 PR과 임시 브랜치는 검증 후 모두 닫고 삭제. 테스트용 흔적은 어느 저장소에도 남아있지 않음.
...
review·poll 모드, Claude 프로바이더, 범용 OpenAI 호환 프로바이더(NVIDIA NIM 포함) — 구현·테스트·E2E 검증 완료. 미완료: OpenAI 프로바이더는 `extraBody` 외 OpenAI 전용 기능 없는 범용 버전에 머묾, 이 저장소가 실제 프로덕션 저장소에 상시 배포된 적은 없음 — 위 E2E 실행은 의도적으로 진행하고 정리까지 마친 검증용 실행.
