...
**대형 diff 검증 (`fetchPrDiff` 재작성 이후):** 원래 `fetchPrDiff.ts`는 `GET /pulls/:number`에 diff 미디어 타입으로 요청했는데, 이 엔드포인트가 문서화 안 된 ~300파일 / ~20,000줄 한계를 넘으면 406을 뱉는다. 호출부는 이 에러를 (리뷰 실패가 PR을 막지 않도록 의도적으로) `core.warning`으로 삼켜버려서, 두 한계 중 하나라도 넘는 PR은 조용히 리뷰 댓글 0개로 끝났다. `pulls.listFiles` 페이지네이션으로 바꿔 해결(한계 훨씬 높고 줄 수 제한 없음). 재검증 내역:

- feed-flow #22 — 실제 297파일 / 35,000줄 PR, 리뷰 댓글 5개 정상 게시.
- feed-flow #24 — 원래 실패했던 커밋과 같은 모양의 합성 191파일 / 23,493줄 PR — 워크플로가 `core.warning` 없이 정상 완료, diff fetch 자체가 옛 줄 수 한계를 더 이상 넘지 않는다는 것 확인. 검증 후 닫고 삭제.
...
review·poll 모드, Claude 프로바이더, 범용 OpenAI 호환 프로바이더(NVIDIA NIM 포함)까지 구현·테스트·E2E 검증을 마쳤다. feed-flow 기본 브랜치에 상시 배포 완료(테스트용 1회성 PR이 아니라 실제 `worktrace.yml`이 돌아가는 중).

미완료는 두 가지다. OpenAI 프로바이더는 아직 `extraBody` 외에 OpenAI 전용 기능이 없는 범용 버전에 머물러 있다. 그리고 더 다양한 저장소·PR 형태에 대한 장기 프로덕션 검증은 아직 남아 있다.
